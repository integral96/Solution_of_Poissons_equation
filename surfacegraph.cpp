...
constexpr int sampleCountX = 10;
constexpr int sampleCountZ = 10;
constexpr int heightMapGridStepX = 4;
constexpr int heightMapGridStepZ = 4;
const float sampleMin = -16.0f;
const float sampleMax = 16.0f;

static auto func_g([](auto x, auto y) { return 2*std::sqrt(x*x + y*y); });
static auto func_phi([](auto x, auto y) { return 0.235*(x + y); });
...
{
    PUASSON_SOLVER<sampleCountX, sampleCountX, decltype (func_g), decltype (func_phi)> solver(func_g, func_phi, 1., 1.);



    float stepX = (sampleMax - sampleMin) / float(sampleCountX - 1);
    float stepZ = (sampleMax - sampleMin) / float(sampleCountZ - 1);
    for(const auto& [x, y] : solver.FF_1) {
        std::cout << x << ": = ";
        for(const auto& z : y) std::cout << z << ", ";
        std::cout << std::endl;
    }
    std::cout << std::endl;
...
//            float y = (qSin(R) / R + 0.24f) * 1.61f;
            ...
            (*newRow)[index++].setPosition(QVector3D(j, solver.YY_(j).at(i), i));
        }
        *dataArray << newRow;
    }
